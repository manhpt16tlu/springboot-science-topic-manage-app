...
public class TopicController {
    ...
    @PostMapping(value = "/organ/{organId}/topic")
    public ResponseEntity<TopicDto> createTopic(@PathVariable Integer organId, @RequestBody TopicDto topicRequest) {
       return new ResponseEntity<>(topicService.createTopic(organId,topicRequest),HttpStatus.CREATED);
    }
    @GetMapping(value = "/organ/{organId}/topic")
    public ResponseEntity<List<TopicDto>> getTopicsByOrganId(@PathVariable Integer organId){
        return new ResponseEntity<>(topicService.getTopics(organId),HttpStatus.OK);
    }
    @PutMapping(value = "/{id}")
